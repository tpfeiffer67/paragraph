// The multistrings library provides a set of utility functions to work with a slice of strings.
...
// The multistrings library provides a MultiStrings type, which is an alias for a slice of strings.
...
// New creates and returns a new MultiStrings slice with a given capacity.
...
// NewWithGivenLen creates and returns a new MultiStrings slice with a given length.
...
// NewFromStringSlice creates and returns a new MultiStrings slice from a string slice.
// - s is a slice of strings from which to create the new MultiStrings slice.
...
// NewWithPresetContent creates and returns a new MultiStrings slice with a preset line count and content.
...
// WriteToFile writes the MultiStrings slice to a file with a given filename.
...
// ToString concatenates the MultiStrings slice into a string with a given line separator.
...
// Cut truncates the MultiStrings slice to a given maximum width by cutting strings that exceed it.
...
// Limit truncates the MultiStrings slice to a given maximum width by splitting strings that exceed it.
// - maxWidth is the maximum width to which to truncate the strings.
...
// PadRight pads the MultiStrings slice on the right side with a given fill pattern to a given width.
...
// Surround surrounds each line of the MultiStrings slice with a given left and right string.
